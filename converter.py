...
class TData:
    ...
    async def _session_to_tdata(self, session_path: str) -> None:
        ...
        try:
            # print(f"./account/tdata{session_path.split('.')[0]}")
            tdesk.SaveTData(f"./account/tdata{session_path.split('.')[0]}/")
            print('dasd')
        ...
        shutil.copy(f'./sessions_to_tdata/{session_path}', f'./session/{session_path}')
...
async def SessionToTData(file_name):
    tdata = TData()

    for session in os.listdir(CONVERT_SESSION):

        # print(session, file_name)
        if "." in session and session.split(".")[1] == "session" and file_name in session:
            # print('asd')
